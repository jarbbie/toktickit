...
describe("My Tickets", () => {
  it("shows the requester’s ticket and sends filter changes to the API", async () => {
    ...
    const loadTickets = vi.spyOn(api, "loadTickets").mockImplementation(async (query) => ({ items: [{ id: 1, ticketNumber: "TKT-2026-A1B2C3D4", summary: "VPN cannot connect", requestedPriority: "HIGH", itPriority: "HIGH", status: "NEW", category: { id: 2, name: "Hardware" }, createdAt: "2026-08-24T00:00:00.000Z", updatedAt: "2026-08-25T00:00:00.000Z" }], page: query.page ?? 1, pageSize: 10, totalItems: 100, totalPages: 10 }));
    ...
    expect(await screen.findByText("TKT-2026-A1B2C3D4")).toBeInTheDocument();
    expect(loadTickets).toHaveBeenLastCalledWith(expect.objectContaining({ categoryId: "2" }));

    await user.click(screen.getByRole("button", { name: /Sort by Ticket Number/ }));
    await waitFor(() => expect(loadTickets).toHaveBeenLastCalledWith(expect.objectContaining({ sortBy: "ticketNumber", direction: "asc" })));

    await user.click(screen.getByRole("button", { name: "Page 2" }));
    await waitFor(() => expect(loadTickets).toHaveBeenLastCalledWith(expect.objectContaining({ categoryId: "2", page: 2 })));
  });
