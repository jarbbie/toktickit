...
| API-02 | API | AC-03, AC-04 | Ticket creation, validation, references, duplicate retry, malformed JSON, and oversized JSON | Safe `201` or documented safe error; invalid ticket is not stored | `server/tests/lab-02/create-ticket.api.test.ts` | Pass |
| API-03 | API | AC-06, AC-08, AC-09 | Owned list search/filter/stable sort/page response | Matching, deterministically ordered owned page and metadata | `server/tests/lab-02/my-tickets.api.test.ts` | Pass |
| API-04 | API | AC-07, AC-10 | Owned detail succeeds; cross-requester detail returns 404 | Owned `200`; unowned `404` | `server/tests/lab-02/ticket-detail.api.test.ts` | Pass |
| API-05 | API | AC-11, AC-12, AC-13 | Attachment upload limits, ownership, removal, compensation, and blocked download | Documented status; removed attachment cannot download | `server/tests/lab-02/attachments.api.test.ts` | Pass |
...
| UI-02 | UI | AC-03–AC-05, AC-15 | Create form validation, optional attachment, busy, success, and retained failure values | Field/file messages; one submission; success or retained Ticket/file | `client/tests/lab-02/CreateTicket.test.tsx` | Pass |
...
| UI-04 | UI | AC-10–AC-13 | Read-only detail and attachment action states | Visible detail; active download; removed metadata; safe attachment failure | `client/tests/lab-02/RequesterTicketDetail.test.tsx` and `client/tests/lab-02/AttachmentSection.test.tsx` | Pass |
...
| E2E-01 | E2E | AC-03–AC-06, AC-08–AC-15 | Requester exercises Create states, creates with an attachment, finds/opens the Ticket, demonstrates list controls and states, adds/downloads/removes another attachment, and captures evidence | Created ticket and attachment are found; list filtering/sorting/pagination work; active file downloads; removed link is absent | `client/e2e/lab-02/requester-ticket-flow.spec.ts` | Pass |
...
| Required state evidence | `states/` contains requester loading/empty/failure; Create validation, invalid-attachment, submitting, success, and safe API-failure; My Tickets empty, no-results, filter/sort/pagination; requester-switch; and cross-requester Ticket/Attachment denial screenshots. Pass. |
...
- `cd client && npm run test:e2e` — 2 tests passed; it seeds the database,
  verifies an active attachment download filename, and creates the then-current
...
Final compliance correction completed on `fix/lab2-final-compliance`:

- `cd server && npx prisma migrate status` — 2 migrations found; database
  schema up to date.
- `cd server && npm run prisma:seed` — passed; seed remains idempotent.
- `npm test --prefix server` — 33 tests passed.
- `npm run build --prefix server` — passed.
- `npm test --prefix client` — 19 tests passed.
- `npm run build --prefix client` — passed.
- `npm run test:e2e --prefix client` — 2 tests passed and generated 28
  screenshots, including active-download, validation, loading, empty, failure,
  requester-switch, and cross-requester denial evidence.
